...
initialise()
{
  ...
  else
  {
    ...
    file_grabber_.initialize(params_.path_str,
                             ...
                             params_.right_img,
                             params_.disp_img,
                             true, //params_.right_img,
                             &file_grabber_mon_);


   file_grabber_.getFileName(fileName);
...
template <class Camera>
void FrameGrabber<Camera>::getFileName(vector<string> & namelist){
	for(int i=0;i<this->fileName.size();i++)
		{namelist.push_back(this->fileName[i]);}
}
...
processNextFrame()
{
  ...
  cout<<"frame_data.frame_id "<<frame_data.frame_id << endl;
  if (!params_.livestream)
  {
    FrameBundle bundle;
    ...
    if (params_.disp_img)
    {
      cout<<"frame grabber get disp"<<endl;
    ...
    else if (params_.depth_img)
    {
      ...
      //frame_data.right.uint8 = cv::imread(right_sstr.str(),0);
    	cout<<"frame grabber get depth"<<endl;
