...
void FileGrabber::getFileName(vector<string> & namelist){
	for(int i=0;i<this->file_base_vec_.size();i++)
		{namelist.push_back(this->file_base_vec_[i]);}
}
...
operator()                ()
{
  while(initialized_)
  {
    if (mon_->isBufferFull()==false)
    {
      ...
      if (get_disp_)
      {
    	  //cout<<"get disp img"<<endl;
    	  bundle.disp = getDispImage();
      }

      if (get_depth_)
      {
    	 // cout<<"get depth img"<<endl;
