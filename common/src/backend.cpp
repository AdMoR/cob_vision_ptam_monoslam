...
::cloneDrawData() const
{
  ...
  draw_data->active_line_set = graph_.active_line_set();
...
::computeNeighborhood(int root_id) const
{
  ...
  addLinesToNeighbors(neighborhood_ptr.get());
...
void Backend
::addLinesToNeighbors(     Neighborhood * neighborhood) const


{
	neighborhood->tracked_lines_result = graph_.line_table();
	//cout<<"addLinesToNeighbors, size of line table: "<<neighborhood->tracked_lines_result.size()<<endl;
}
void Backend
    ...
    Neighborhood * neighborhood) const
{
  ...
  //    cout<<"computing neighborhood, addPointsToNeighbors"<<endl;
...
::globalLoopClosure(const DetectedLoop & loop)
{
  ...
  const Frame & loop_frame = GET_MAP_ELEM(loop.loop_keyframe_id, keyframe_map_);
  const StereoGraph::Vertex & v_query = GET_MAP_ELEM(loop.query_keyframe_id, graph_.vertex_table());
  const StereoGraph::Vertex & v_loop = GET_MAP_ELEM(loop.loop_keyframe_id, graph_.vertex_table());
  SE3 T_loop_from_world = loop.T_query_from_loop.inverse()*v_query.T_me_from_world;
  ...
  //For all the point features
  for (ImageFeature<3>::Table::const_iterator it = v_query.feature_table.begin(); it!=v_query.feature_table.end(); ++it)
  {
    int point_id = it->first;
    const StereoGraph::Point & p = GET_MAP_ELEM(point_id, graph_.point_table());

    //TODO: include anchor verteces not in double window??
    //If the anchor frame of the point is in the double win
    if (IS_IN_SET(p.anchorframe_id, graph_.double_window())==false)
    {
      continue;
    }

    const StereoGraph::Vertex & v_anchor = GET_MAP_ELEM(p.anchorframe_id, graph_.vertex_table());
    ...
    //Get the reestimated proj
    Vector2d uv_pyr = cam_pyr.map(project2d(xyz_loop));

    //If it falls into the frame
    if (cam_pyr.isInFrame(uv_pyr.cast<int>(), 0)==false)
      continue;

    //Add the point
    candidate_point_list.push_back(CandidatePoint3Ptr( new CandidatePoint<3>(point_id,
                                                           p.xyz_anchor,
                                                           p.anchorframe_id,
                                                           p.anchor_obs_pyr,
                                                           p.anchor_level,
  ...
  //TODO Same workflow with lines

  TrackData<3> track_data;

  SE3 T_newloop_from_oldloop;
  if (matchAndAlign(loop_frame, loop.loop_keyframe_id, vertex_table,candidate_point_list,&T_newloop_from_oldloop, &track_data) == false)
    return false;
  ...
  for (list<IdObs<3> >::const_iterator it = track_data.obs_list.begin(); it!=track_data.obs_list.end(); ++it)
  {
    IdObs<3> id_obs = *it;
    const Vector3d & point = track_data.point_list.at(id_obs.point_id);
    const Vector3d & uvu_pred = se3xyz_stereo_.map(T_newloop_from_oldloop, point);
    const Vector3d & uvu = id_obs.obs;
    Vector3d diff = uvu - uvu_pred;

    StereoGraph::MyActivePointPtr point_ptr  = GET_VEC_VAL(id_obs.point_id, track_data.ba2globalptr);
    int factor = zeroFromPyr_i(1, point_ptr->anchor_level);
        ...
        && abs(diff[2])<REPROJ_THR*3)
    {
      ...
      ImageFeature<3> feat(uvu, point_ptr->anchor_level);
      StereoGraph::MyTrackPointPtr trackpoint( new StereoGraph::MyTrackPoint(global_point_id,feat));
