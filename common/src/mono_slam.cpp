...
//float g_focal_length=591.524,g_baseline=0.07468,g_doff=1090;
//float g_focal_length=580.,g_baseline=7.5,g_doff=1090;
//float g_focal_length=532.,g_baseline=0.075,g_doff=1090;
//float g_focal_length=523.,g_baseline=0.075,g_doff=1090;
float g_focal_length=523.,g_baseline=7.5,g_doff=1090;
//float g_focal_length=520.,g_baseline=0.075,g_doff=1090;
             ...
             Views * views, ros::NodeHandle & nh)
{
  ...
  modules.frame_grabber->initialise(); //get frames data
  modules.frontend =
      new MonoFrontend(&modules.frame_grabber->frame_data, modules.per_mon);
  modules.frontend->initialize(nh);
...
void draw(int loop_id,
          Views * views,
          Modules * modules,
          BackendDrawDataPtr & graph_draw_data)
{
  ...
  for (int level = 0; level<NUM_PYR_LEVELS; ++level)
  {
    ...
    if (ui_show_tracked_points)
    {
           ...
           it!=line_list.end(); ++it)
      {
        ...
        //cout<<"tracked point p1: "<<line.p1<<" p2:"<< line.p2<<endl;
  ...
  if (ui_show_optimized_points)
  {
         ...
         ++it_win1)
    {
     ...
     // cout<<"scavislam draw pose id: "<<pose_id_1<<endl;
 ...
 //   cout<<"drawing T_world_from_actkey"<<endl;
    Draw3d::pose(
          T_actkey_from_world.inverse());
  }
  glColor3f(0,0,1);
 ...
 // cout<<"drawing T_world_from_cur"<<endl;
...
void getCurrentMapToFile(StereoGraph & sg,string fileName){

	StereoGraph::VertexTable vtable = sg.vertex_table();

	for(auto ptr=vtable.begin();ptr!=vtable.end();ptr++){
		//(*((*ptr).second)).
		SE3 T_cur_from_world=(*((*ptr).second)).T_me_from_world;
		tf::Vector3 v(T_cur_from_world.matrix()(0,3),T_cur_from_world.matrix()(1,3),T_cur_from_world.matrix()(2,3));
		tf::Quaternion q(T_cur_from_world.so3().unit_quaternion().x(),T_cur_from_world.so3().unit_quaternion().y(),T_cur_from_world.so3().unit_quaternion().z(),T_cur_from_world.so3().unit_quaternion().w());
		dumpToFile(std::to_string((*((*ptr).second)).own_id), v.x(), v.y(), v.z(), q.x(), q.y(), q.z(), q.w(), "/home/rmb-am/Slam_datafiles/map/map.txt");

	}

	dumpToFile("<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>",0,0,0,0,0,0,0, "/home/rmb-am/Slam_datafiles/map/map.txt");

}

void publishPoses(BackendDrawDataPtr & graph_draw_data, ros::Publisher & marker_pub, ros::Publisher & err_pub, tf::TransformBroadcaster & br, Modules & modules, string frame_name="")
{
	 SE3 T_actkey_from_world;
	  if (IS_IN_SET(modules.frontend->actkey_id,
	                modules.frontend->neighborhood()->vertex_map))
	  ...
	  SE3 T_cur_from_world
	        = modules.frontend->T_cur_from_actkey()*T_actkey_from_world;
	  SE3 T_world_from_cur = T_cur_from_world.inverse();

//	  cout<<"T_world_from_cur: "<<T_world_from_cur.matrix()<<endl;

	  tf::Transform tr ;
	  tf::Vector3 v=tf::Vector3(T_world_from_cur.matrix()(0,3),T_world_from_cur.matrix()(1,3),T_world_from_cur.matrix()(2,3));
	  tf::Quaternion q(T_world_from_cur.so3().unit_quaternion().x(),T_world_from_cur.so3().unit_quaternion().y(),T_world_from_cur.so3().unit_quaternion().z(),T_world_from_cur.so3().unit_quaternion().w());
	  tr.setOrigin( v );
	  tr.setRotation( q);
	  ...
	  transform.setOrigin(tf::Vector3(T_cur_from_world.matrix()(0,3),T_cur_from_world.matrix()(1,3),T_cur_from_world.matrix()(2,3)));//(tf::Quaternion(q.x(),q.y(),q.z(),q.w()),tf::Vector3(v.x(),v.y(),v.z()));

	  msgpkg::errmsg m;
	  m.frame_nb=frame_name;
	  m.x=v.x();
	  m.y=v.y();
	  m.z=v.z();
	  m.rx=q.x();
	  ...
	  //cout << T_world_from_cur.translation() << "hfbejhg " << T_world_from_cur.so3().unit_quaternion() << "sjukfjnekfhnciekfuj" << endl;
	//transform.setOrigin( tf::Vector3(T_world_from_cur.translation()) );


	//br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "/world", "/mono"));

	visualization_msgs::Marker point;
	point.header.frame_id = "/mono";
	...
	point.lifetime=ros::DURATION_MAX;
	marker_pub.publish(point);


	br.sendTransform(tf::StampedTransform(tr, ros::Time::now(), "/world", "/currentPose"));
...
std::string cfg;
  ...
  if (ros::param::has("~cfgpath"))
  {
    ros::param::get("~cfgpath", cfg);
    cout<<"read param file: "<<cfg<<endl;
  }
  else
  {
    cout << "please the name of the configuration file! For example cfgpath:=rgbd_example.cfg" << endl;
    exit(0);
  }
  ros::NodeHandle nh,nh_err;
  ros::Publisher marker_pub = nh.advertise<visualization_msgs::Marker>("visualization_marker", 1);
  ros::Publisher err_pub = nh_err.advertise<msgpkg::errmsg>("err_calc", 1);
  static tf::TransformBroadcaster br;

  pangolin::ParseVarsFile(cfg);
  ...
  modules.frame_grabber->getFileName(frameNames);
  ...
  while(true)
  {
                  ...
                  || show_next_frame) )
    {
      ...
      modules.frame_grabber->processNextFrame();//addNewKeyframe()->getNewUniqueId() generates new actkey_id
      frame_nb+=1;
      NeighborhoodPtr neighborhood;
      modules.backend->monitor.queryNeighborhood(modules.frontend->actkey_id); //push id to query_frame_id_storage_ stack
      ...
      {
        if (neighborhood->vertex_map.find(modules.frontend->actkey_id) != neighborhood->vertex_map.end())
        {
    ...
    //Get cuurent Kf graph
    StereoGraph sg=(*(modules.backend)).getGraph();
    if(publish_data)
    	getCurrentMapToFile(sg,"map");

    draw(best_match, &views, &modules, graph_draw_data);

    if(publish_data){
    	if(frameNames.size()>frame_nb)
    		publishPoses(graph_draw_data, marker_pub, err_pub, br, modules,frameNames[frame_nb].substr(175,17));
    	else
    		publishPoses(graph_draw_data, marker_pub, err_pub, br, modules,"No value");
    }
