  ...
  : stereo_cam_(stereo_cam)
{
  ...
  if (ros::param::has("~surfwordspath"))
  {
    ros::param::get("~surfwordspath", image_path);
    cout<<"read param file: "<<image_path<<endl;
  }
    else
  {
    cout << "couldn't find param var with the path to the surfwordsimage" << endl;
    exit(0);
  }
  cv::Mat words_float_as_four_uint8
      = cv::imread(image_path,-1);
     if(! words_float_as_four_uint8.data )                              // Check for invalid input
{
