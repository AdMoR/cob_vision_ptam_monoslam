...
/*void camera_info_cb(const sensor_msgs::CameraInfoConstPtr& msg)
{
	...
	cout << "camera info received"<<endl;
}*/
...
std::string cfg;
  ...
  Modules modules = startModules(stereo_camera,
                                 &views);
  //ros::Subscriber camera_info = nh.subscribe("/camera/rgb/camera_info", 1, camera_info_cb);
  ...
  while(true)
  {
	//if( camera_info_received) camera_info.shutdown();
