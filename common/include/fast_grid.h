...
public:
  ...
  detectAdaptively           (const cv::Mat & img,
                              int trials,
                              QuadTree<int> * qt);/* base decision of adding candidate feature z to the image on how many features are already in the bounding
  box around z*/

  void detectAdaptivelyPTAM( KeyFrame *kf,
          int trials,
          QuadTree<int> * qt);
